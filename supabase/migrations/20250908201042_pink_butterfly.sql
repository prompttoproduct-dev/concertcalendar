/*
  ...
  1. New Columns
    - `external_id` (text, unique identifier from external APIs)
    - `title` (text, event title from API)
    - `image_url` (text, event image URL)
    - `source` (enum, data source - manual, ticketmaster, eventbrite)
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'source_type') THEN
    CREATE TYPE source_type AS ENUM ('manual', 'ticketmaster', 'eventbrite');
  END IF;
END $$;
...
BEGIN
  ...
  UPDATE concerts SET price = regexp_replace(price, '[^0-9.]', '', 'g') WHERE price ~ '[^0-9.]';
  
  -- Set empty or invalid prices to 0
  UPDATE concerts SET price = '0' WHERE price = '' OR price IS NULL;
...
WHERE title IS NULL AND source = 'manual';